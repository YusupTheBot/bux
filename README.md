...
SSH into it (I'll paste the host below), run install.sh, and optionally wire up a Telegram bot if I give you a token from @BotFather. Read install.md first. After the install completes, verify the services are running (systemctl is-active bux-browser-keeper bux-ttyd), then sync my viberelay profile to the box and test the setup by asking the agent to visit https://browser-use.com and report the page title.
```
...
- When the browser hits a login wall / 2FA / CAPTCHA, the agent stops and asks you to act in the real local Chrome session instead of faking around it

## Requirements

- **A box** — Ubuntu 22.04+ with ≥2GB RAM. A $5/mo VPS is fine.
- **Local Chrome or Chromium** available on the box, or a reachable local CDP endpoint at `http://127.0.0.1:9222`.
...
```
  telegram ──►  telegram_bot.py ─┐
                                 ├──► viberelay/claude -p  ──► browser-harness ──► local Chrome CDP
  browser  ──►  ttyd ────────────┘              │                    (127.0.0.1:9222)
                                                ▼
